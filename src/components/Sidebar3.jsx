...
export default function Sidebar() {
  ...
  // useAuth returns an object { isAuthenticated, login, logout }
  const { isAuthenticated } = useAuth();

  // Dropdown / modal state (these were referenced in your effects but not defined)
  ...
  const [changePwdFeedback, setChangePwdFeedback] = useState({
    success: false,
    error: false,
  });
  ...
  useEffect(() => {
    function handleClickOutside(e) {
      if (
        ...
        // allow clicks on profile button to toggle
        !(profileSectionRef.current && profileSectionRef.current.contains(e.target))
      ) {
        setDropdownOpen(false);
      }
    }
  ...
  async function handleChangePassword(e) {
    e.preventDefault();
    // Implement real password change logic here.
    // We'll simulate success after small delay:
    try {
      setChangePwdFeedback({ success: false, error: false });
      // simulate network
      await new Promise((r) => setTimeout(r, 600));
      setChangePwdFeedback({ success: true, error: false });
      setChangePasswordModalOpen(false);
    } catch (err) {
      setChangePwdFeedback({ success: false, error: true });
    }
  }
    ...
    <>
      ...
      >
        ...
        <div>
          <nav className="flex flex-col space-y-6 items-center">
            <Link
              to="/"
              className="to-home w-12 h-12 flex items-center justify-center rounded-full bg-gray-200 text-gray-800"
              title="Home"
              aria-label="Home"
            >
              <i className="fas fa-home text-xl" aria-hidden />
            </Link>
            ...
            <Link
              ...
              title="History"
              aria-label="History"
              onMouseEnter={() => {setIsLibraryHovered(true);fetchThreads(); }}
        ...
        {/* Bottom user profile section: only show when authenticated */}
      ...
      >
        <div className="p-4 space-y-4">
          ...
          <span
            className="block px-3 py-2 text-sm font-semibold text-gray-700"
          >
            Recent
          </span>
